...
minetest.register_craftitem("boat_test:flowstick_8d", {
	...
	--liquids_pointable = true,
	on_place = function(itemstack, placer, pointed_thing)
		minetest.chat_send_all(quick_8d_water_flow(pointed_thing.above).x .. " , " .. quick_8d_water_flow(pointed_thing.above).z)
	end,
	on_use = function(itemstack, user, pointed_thing)
...
minetest.register_craftitem("boat_test:flowstick", {
	...
	--liquids_pointable = true,
	on_place = function(itemstack, placer, pointed_thing)
		minetest.chat_send_all(quick_water_flow(pointed_thing.above).x .. "," .. quick_water_flow(pointed_thing.above).z)
	end,
	on_use = function(itemstack, user, pointed_thing)
